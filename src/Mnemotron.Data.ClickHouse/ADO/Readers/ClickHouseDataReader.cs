...
namespace Mnemotron.Data.ClickHouse.ADO.Readers;
...
public class ClickHouseDataReader : DbDataReader, IEnumerator<IDataReader>, IEnumerable<IDataReader>, IDataRecord
{
    ...
    // The type name is the verbatim string from the response header
    // (RowBinaryWithNamesAndTypes), not a reconstruction from the parsed type.
    public override string GetDataTypeName(int ordinal) => RawTypeNames[ordinal];
