...
using Mnemotron.Data.ClickHouse.Constraints;
using Mnemotron.Data.ClickHouse.Copy;
using Mnemotron.Data.ClickHouse.Tests.Attributes;
using Mnemotron.Data.ClickHouse.Utility;
using NUnit.Framework;

namespace Mnemotron.Data.ClickHouse.Tests.BulkCopy;
...
public class BulkCopyWithDefaultsTests : AbstractConnectionTestFixture
{
    private static IEnumerable<TestCaseData> Get()
    {
        ...
        // The server evaluates toDate(now()) in its own timezone (UTC in docker-compose.yml);
        // the client's local date differs near midnight, so compare against the UTC date.
