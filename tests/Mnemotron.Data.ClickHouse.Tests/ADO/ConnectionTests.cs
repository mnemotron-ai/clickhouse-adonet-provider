...
public class ConnectionTests : AbstractConnectionTestFixture
{
    ...
    {
        // No collection name = MetaDataCollections (ADO.NET convention)
        var schema = connection.GetSchema();
        ClassicAssert.NotNull(schema);
        Assert.That(GetColumnNames(schema), Is.EquivalentTo(new[] { "CollectionName", "NumberOfRestrictions", "NumberOfIdentifierParts" }));
    }
    ...
    public void ShouldFetchSchemaTables()
    {
        ...
        ClassicAssert.NotNull(schema);
        Assert.That(GetColumnNames(schema), Is.EquivalentTo(new[] { "TABLE_CATALOG", "TABLE_SCHEMA", "TABLE_NAME", "TABLE_TYPE" }));
    }
    ...
    public void ShouldFetchSchemaDatabaseColumns()
    {
        ...
        ClassicAssert.NotNull(schema);
        Assert.That(new[] { "TABLE_CATALOG", "TABLE_SCHEMA", "TABLE_NAME", "COLUMN_NAME", "DATA_TYPE" }, Is.SubsetOf(GetColumnNames(schema)));
    }
    ...
    public void ShouldFetchSchemaTableColumns()
    {
        ...
        ClassicAssert.NotNull(schema);
        Assert.That(new[] { "TABLE_CATALOG", "TABLE_SCHEMA", "TABLE_NAME", "COLUMN_NAME", "DATA_TYPE" }, Is.SubsetOf(GetColumnNames(schema)));
        Assert.That(schema.Rows.Cast<DataRow>().Select(r => (string)r["TABLE_NAME"]), Is.All.EqualTo("functions"));
    }
