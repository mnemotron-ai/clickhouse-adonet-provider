...
public class DataReaderTests : AbstractConnectionTestFixture
{
    ...
    // GetDataTypeName must return the verbatim server type string
    // (RowBinaryWithNamesAndTypes header), not a reconstruction from the parsed type.
    [Test]
    [TestCase("toDecimal32('123.45', 2)", "Decimal(9, 2)")]
    [TestCase("toDecimal256('1.5', 6)", "Decimal(76, 6)")]
    [TestCase("toDateTime('2024-06-15 12:34:56')", "DateTime")]
    [TestCase("toDateTime('2024-06-15 12:34:56', 'UTC')", "DateTime('UTC')")]
    [TestCase("toDateTime64('2024-06-15 12:34:56.789', 3)", "DateTime64(3)")]
    [TestCase("CAST('a' AS Enum8('a' = 1, 'b' = 2))", "Enum8('a' = 1, 'b' = 2)")]
    [TestCase("tuple(1, 'x')", "Tuple(UInt8, String)")]
    public async Task ShouldGetCanonicalServerDataTypeName(string expression, string expectedTypeName)
    {
        using var reader = await connection.ExecuteReaderAsync($"SELECT {expression} AS value");
        ClassicAssert.IsTrue(reader.Read());
        Assert.That(reader.GetDataTypeName(0), Is.EqualTo(expectedTypeName));
    }
