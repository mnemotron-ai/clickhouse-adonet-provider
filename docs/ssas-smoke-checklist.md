...
| 2 | DSV wizard reads tables/columns | Data Source View Wizard → the ClickHouse table tree renders (GetSchema: Tables/Columns), selecting 2–3 tables builds a DSV | ☐ | 2026-07-20 first live run: **empty object tree.** Root cause found (server query_log had zero `system.tables` queries; a standalone `GetSchema("Tables", 4 restrictions)` repro threw): the wizard passes SqlClient-shaped restriction arrays (Tables/Columns = 4, Views = 3), alpha.5 declared 2/2/3 and threw `ArgumentException` in `CheckRestrictionCount`; SSDT swallows the exception and renders an empty tree. Fixed in WS-2 follow-up (SqlClient shapes + extras ignored); re-run pending |
